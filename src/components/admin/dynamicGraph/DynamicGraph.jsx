...
const DynamicGraph = () => {
    ...
    const doctorAndPatientCount = async () => {
        ...
        try {
            const response = await axios.get(`https://doctorbackend-je62.onrender.com/getTotalPatientByDoctor`, { headers: { 'Authorization': `${token}` } });
    ...
    const dateAndPatientCount = async () => {
        const token = localStorage.getItem('authToken');
        try {
            const response = await axios.get(`https://doctorbackend-je62.onrender.com/getTotalPAtientByDate`, { headers: { 'Authorization': `${token}` } });
